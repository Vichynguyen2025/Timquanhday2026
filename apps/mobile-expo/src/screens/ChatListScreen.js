      ...
      setConversations((prev) => {
        const updated = prev.map((c) =>
          c.id === msg.conversation_id
            ? { ...c, last_message: msg.content || (msg.type === "image" ? "📷 Ảnh" : c.last_message), last_message_at: msg.created_at, unread_count: c.unread_count + 1 }
            : c
        );
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingTop: 60, paddingBottom: 8, backgroundColor: colors.surface },
  ...
  convItem: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border + "50" },
  ...
  onlineDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: colors.online, borderWidth: 2, borderColor: "#fff", position: "absolute", bottom: 0, right: 0 },
  convInfo: { flex: 1, marginLeft: 12 },
  convTop: { flexDirection: "row", justifyContent: "space-between" },
  convName: { fontSize: 14, fontWeight: "600", color: colors.text, flex: 1 },
  convTime: { fontSize: 11, color: colors.textTertiary },
  convLast: { fontSize: 13, color: colors.textSecondary, marginTop: 2 },
  unread: { backgroundColor: colors.primary, borderRadius: 10, minWidth: 20, height: 20, alignItems: "center", justifyContent: "center", marginLeft: 8, paddingHorizontal: 6 },
